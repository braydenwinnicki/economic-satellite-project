...
# .mean() = average of the column; .std() = standard deviation (spread) of the column
mean_income = df_train["median_income"].mean()
std_income = df_train["median_income"].std()

# (value - mean) / std  shifts values so the average becomes 0 and the spread becomes 1
df_train["median_income"] = (df_train["median_income"] - mean_income) / std_income

# Apply the exact same transformation to test data using train's stats
# If you used test.mean() here you'd be cheating — leaking info from test into training
...
for epoch in range(epochs):
    ...
    # Each iteration of this inner loop processes one batch of 32 images
